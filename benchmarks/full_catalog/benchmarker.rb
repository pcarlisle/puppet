...
class Benchmarker
  ...
  def setup
    ...
    %x(bundle exec r10k puppetfile install --puppetfile #{File.join(environment, 'Puppetfile')} --moduledir #{File.join(environment, 'modules')} --config #{File.join('benchmarks', 'full_catalog', 'r10k.yaml')})
  end
